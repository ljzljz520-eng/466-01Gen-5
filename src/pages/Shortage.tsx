...
import { PackageOpen, AlertTriangle, ArrowRightLeft, Truck, Plus } from 'lucide-react'
...
export default function Shortage() {
  ...
  const { shortages, drugs, patients, addShortage, addSubstitute, restockShortage } = store
  ...
  const active = filter ? shortages.filter((s) => s.status === filter) : shortages.filter((s) => s.status !== 'restocked')
  ...
  const affectedPatients = (shortageId: string) => {
    ...
    s.substitutes.forEach((sub) => sub.patientIds.forEach((pid) => pids.add(pid)))
    store.prescriptions.forEach((rx) => {
      if (rx.status !== 'active') return
      rx.items.forEach((item) => {
        if (item.drugId === s.drugId) pids.add(rx.patientId)
      })
    })
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-rose-600 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-rose-700 transition-colors shadow-sm"
          >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-1 text-xs text-slate-400">
                      <span>缺货数量：{s.shortageQuantity}{drug?.unit || '盒'}</span>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {s.status !== 'restocked' && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-teal-600 text-white hover:bg-teal-700 transition-colors"
                    >
              ...
              {affected.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {affected.map((pid) => (
                      <span key={pid} className="text-xs px-2 py-0.5 rounded-full bg-slate-50 text-slate-600">
              ...
              {s.substitutes.length > 0 && (
                <div className="mt-3 pt-3 border-t border-slate-100">
                  ...
                  {s.substitutes.map((sub) => {
                    ...
                    return (
                      <div key={sub.id} className="flex items-center gap-2 text-sm bg-amber-50 rounded-lg p-2.5 mb-1.5">
        ...
        <div className="space-y-4">
          <div>
            ...
            >
              ...
              {drugs.map((d) => (
                <option key={d.id} value={d.id}>{d.name}（{d.specification}）当前库存：{d.stock}{d.unit}</option>
              ))}
            </select>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              ...
              className="px-4 py-2 text-sm text-white bg-rose-600 rounded-lg hover:bg-rose-700 transition-colors font-medium"
            >
              确认登记
            </button>
          </div>
          ...
          <div>
            ...
            >
              ...
              {drugs.filter((d) => d.stock > 0).map((d) => (
                <option key={d.id} value={d.id}>{d.name}（{d.specification}）库存：{d.stock}{d.unit}</option>
              ))}
            </select>
          </div>
          <div>
